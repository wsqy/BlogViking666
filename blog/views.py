...
def global_setting(request):
    ...
    # 评论排行
    clickCount_list = Article.objects.filter(handle=True).order_by("-click_count")[:6]
    # 站长推荐
    publish_list = Article.objects.filter(is_publish=True, handle=True).order_by("-modify_date")[:6]
...
def index(request):
    ...
    try:
        ...
        # archive_list = Article.objects.distinct_date()
        # 最新文章,默认按照修改时间排序
        article_list = Article.objects.filter(handle=True).order_by("-create_date")
...
def pigeonhole(request):
    try:
        ...
        article_list = get_list_or_404(Article, create_date__icontains=year + "-" + month, handle=True)
        # article_list = Article.objects.filter(create_date__icontains=year + "-" + month).filter(handle=True)
        article_list = getPage(request, article_list)
    except Exception as e:
        logger.error(e)
    mes_title = "%s年%s月文章" % (request.GET['year'], request.GET['month'])
    return render(request, 'blog/article_list.html', locals())
    # return render(request, 'blog/pigeonhole.html', locals())
...
def tag(request, tag_name):
    try:
        # 首先获取用户提交的数据
        article_list = get_list_or_404(Article, tags__name=tag_name, handle=True)
        # article_list = Article.objects.filter(tags__name=tag).filter(handle=True)
        article_list = getPage(request, article_list)
    except Exception as e:
        logger.error(e)
    mes_title = "%s 标签下的文章" % (tag_name, )
    return render(request, 'blog/article_list.html', locals())
    # return render(request, 'blog/tagList.html', locals())


# 分类视图
def category(request, category_name):
    try:
        article_list = get_list_or_404(Article, category__name=category_name, handle=True)
        # article_list = Article.objects.filter(tags__name=tag).filter(handle=True)
        article_list = getPage(request, article_list)
    except Exception as e:
        logger.error(e)
    ...
    return render(request, 'blog/article_list.html', locals())
    # return render(request, 'blog/categoryList.html', locals())
...
def search(request):
    ...
    try:
        ...
        # 根据用户输入做个简单的标题模糊匹配
        article_list = Article.objects.filter(title__icontains=search_field)
        article_list = getPage(request, article_list)
